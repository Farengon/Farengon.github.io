...
不仅仅是 Java，所有的编程语言都遵循一定的语法。语法错误可以被编译器找到。存在语法错误的程序无法运行，尝试运行带有语法错误的程序时会得到编译器的报错，可以利用这一点来检查语法错误。
...
逻辑错误是指算法或程序中存在的缺陷，导致其行为异常或出乎意料。此类错误可通过使用特定数据**测试**程序来检测。
