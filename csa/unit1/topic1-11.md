...
`Math` 类是 Java 标准库（`java.lang` 包）中的类，**`java.lang` 包中的类默认可用**，无需 import 语句。`Math` 类只包含**类方法（static 方法）**，通过 `Math.方法名()` 调用。AP CSA 考试要求的 Math 类方法（均为 Java Quick Reference 中的内容）：
