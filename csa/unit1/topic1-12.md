...
**继承层次（Class Hierarchy）：** Java 中，如果多个类有共同的属性和行为，可以把它们放入一个**超类（superclass）**，其他类（**子类 subclass**）可以继承（extends）它。**Java 中所有类都是 `Object` 类的子类**。

{: .note}
> **Exclusion Statement：** 设计并实现继承关系不在 AP CSA 考试范围内——但要知道"对象是类的实例"和"所有类都是 Object 的子类"这两个基本事实。

**引用变量（Reference Variable）：** 引用类型的变量保存的是对象的**引用（内存地址）**，而不是对象本身。若变量不指向任何对象，其值为 `null`。
...
| 继承层次       | 超类/子类概念；所有类都是 Object 的子类      | 设计继承不在考试范围，但要知道基本事实             |
| 引用变量       | 保存对象引用（内存地址），无对象时为 null    | 引用类型变量 ≠ 对象本身                           |
