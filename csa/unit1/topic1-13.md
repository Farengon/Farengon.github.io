...
>
> 引用类型变量要么保存**对象引用**，要么在没有任何对象时保存 **`null`**。

## 1.13.B 构造方法的重载与参数匹配

**构造方法签名（constructor signature）** 由构造方法名（与类名相同）和**有序的参数类型列表**组成。一个类可以有**多个构造方法**（称为**重载 overload**），它们的**参数数量或类型不同**。创建对象时，实参必须与某个构造方法的签名匹配：
