...
**Postcondition（后置条件）** 是方法执行后**必须恒为真**的条件，描述方法执行的**结果**（返回什么、或实例变量处于什么状态）。例如，一个 `getAge` 方法的后置条件可以是"返回值为该对象的年龄"。

{: .note}
> 前置条件（Precondition）是方法**调用前**必须满足的条件；后置条件（Postcondition）是方法**调用后**必须成立的结果。两者都常用 Javadoc 注释描述。
...
| Precondition       | 方法**调用前**必须为真的条件                   | 通常写在方法上方，用 `/** ... */` 描述               |
| Postcondition      | 方法**调用后**必须为真的结果                   | 描述返回什么或实例变量状态                           |
