...
## 2.1.A 顺序、选择与重复（Sequencing, Selection, and Repetition）

算法的**三个构建块（building blocks）**：

- **顺序（Sequencing）**：步骤按顺序一次执行一步。
- **选择（Selection）**：根据条件（真/假）决定执行哪些步骤，用 `if` 语句实现。例如："如果 x 大于 y，则交换 x 和 y"。
- **重复（Repetition）**：反复执行一组步骤，直到达到期望的结果，用 `while` / `for` 循环实现。例如："重复比较并交换，直到所有数有序"。

{: .important}
> 顺序、选择、重复的**使用顺序和组合方式**共同决定算法的输出结果。设计算法时，先想清楚每一步的**条件**和**顺序**，再考虑如何组合三者，往往可以避免逻辑漏洞（例如漏掉最后一个比较）。
