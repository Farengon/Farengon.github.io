...
> - 构造方法用于设置对象的**初始状态（initial state）**——应初始化**所有**实例变量，这体现对象与其属性之间的 **has-a（拥有）** 关系。

{: .note}
> **可变对象作为构造参数（EK 3.4.A.3）：** 若构造方法的参数是**可变对象**（mutable object），实例变量应初始化为该对象的**副本**，而不是直接保存传入的引用。这样外部对原对象的修改不会影响新对象的属性。
