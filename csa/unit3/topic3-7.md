...
> - 类方法内部不能直接使用实例变量（它不属于任何特定对象）——除非通过**参数**传入该类的实例。
> - 用类名调用 → 该方法必须是 static。
> - **`final` 变量**：用 `final` 声明的变量一旦赋值就**不能再被修改**（常量），如 `public static final double PI = 3.14;`。
> - **public 类变量**在类外部通过 `类名.变量名` 访问（如 `Element.maxValue`）。
