...
public static void insertionSort(int[] arr)
{
    ...
    {
        int key = arr[j];          // 取未排序部分的第一个元素
        int k = j - 1;
        while (k >= 0 && arr[k] > key)   // 从右向左移动比 key 大的元素
        {
            arr[k + 1] = arr[k];
            k--;
        }
        arr[k + 1] = key;          // 把 key 放入正确位置
    }
}
```
...
{: .important}
> 选择排序 vs 插入排序：
> - **选择排序**：每轮选**最小值**与当前位置**交换**——该元素到达**最终（final）**位置。
> - **插入排序**：把元素插入已排序部分的正确位置——该元素的位置**不一定是最终位置**（后续插入可能再移动它）。

追踪示例：对 `{5, 2, 4, 6, 1, 3}` 排序，逐步插入：
...
| 插入排序过程       | 逐个插入已排序部分，移动元素腾位             | 插入位置**不一定最终**                         |
| 选择 vs 插入       | 选择=交换到最终位；插入=插入到有序部分       | 大纲只要求选择、插入、归并三种排序             |
